...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($email) || empty($new_password)) {
        ...
        header("Location: resetpass.php?status=invalid_input");
        exit;
    }
    ...
    if (!$stmt) {
        ...
        header("Location: resetpass.php?status=stmt_error");
        exit;
    }
    ...
    if ($stmt->execute()) {
        ...
        header("Location: resetpass.php?status=success");
        exit;
    } else {
        error_log("Error executing query: " . $stmt->error); // Log the error
        header("Location: resetpass.php?status=update_error");
        exit;
    }
